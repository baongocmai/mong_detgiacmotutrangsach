...
const Comments = () => {
      ...
      for (const story of storiesResponse.data) {
        try {
          const chaptersResponse = await axios.get(
            `http://3.26.145.187:8000/api/stories/${story.id}/chapters`
          );
          
          // Verify chapters data exists and is an array
          if (!Array.isArray(chaptersResponse.data)) {
            console.error(`Invalid chapters data for story ${story.id}:`, chaptersResponse.data);
            continue;
          }
          ...
          for (const chapter of chaptersResponse.data) {
            try {
              const commentsResponse = await axios.get(
                `http://3.26.145.187:8000/api/stories/${story.id}/chapters/${chapter.chapter_id}/comments`
              );
          ...
          console.error(`Error fetching chapters for story ${story.id}:`, chapterError);
        }
      }
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(
        `http://3.26.145.187:8000/api/stories/${comment.storyId}/chapters/${comment.chapterId}/comments/${id}`
      );

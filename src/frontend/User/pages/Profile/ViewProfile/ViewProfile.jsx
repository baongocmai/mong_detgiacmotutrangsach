      ...
      const fetchUserData = async () => {
        try {
          ...
          const [profileResponse, favoritesResponse] = await Promise.all([
            ...
            fetch(`http://3.26.145.187:8000/api/favorites`, {
              headers: { "Authorization": `Bearer ${token}` },
            })
          ]);
  ...
  return (
    <div className="view-profile">
      <div className={`profile-container ${isDropdownOpen ? "dropdown-open" : ""}`}>
        <div className={`profile-left ${isDropdownOpen ? "expanded" : ""}`}>
          ...
          <div className={`dropdown-menuu ${isDropdownOpen ? "open" : ""}`}>
            ...
            <button className="dropdown-itemm" onClick={() => navigate("/postStory/:userId")}>
              <FaEdit /> Đăng truyện
            </button>
      ...
      <button className="bun tagbooks">
          <h2 className="section-title" onClick={() => navigate("/myFavouriteStories")}>
            KHO YÊU THÍCH
          </h2>
        </button>
